...
**Aufgaben**
1. Implementiere die Klasse `StudentRoundRobinScheduler` in `src/student/scheduler.py`.
...
2. Führe das Convoy‑Experiment (gleiches Szenario wie Woche 1) mit der mitgelieferten Simulator‑Harness
   aus: `sim/simulator.py`. Vergleiche die Metriken `avg_wait` und `avg_turn` zwischen FIFO und RR.
3. Optional: experimentiere mit verschiedenen `quantum`‑Werten (0.5, 1.0, 2.0) und beobachte die Auswirkungen.

**Dateien / Hinweise**
- Starter‑Code liegt in `src/` (siehe `src/student/scheduler.py` — implementiere dort).
- Notebooks: `notebooks/week02_rr.ipynb` demonstriert die Nutzung der Harness.
- Tests: `tests/test_student_rr.py` und `tests/test_rr_metrics.py`.
